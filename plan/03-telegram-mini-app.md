...
### 1. Экран авторизации
- Поле ввода игрового ника
- Кнопка "Начать"
- После нажатия и успешной валидации ника открывается экран со списком турниров

### 2. Экран "Турниры"
- Список доступных турниров, на которые можно сделать ставку
- При выборе турнира открывается экран турнира

### 3. Экран турнира
- Доступна кнопка "Назад" для возврата к списку турниров
- Две вкладки: "Игры" и "Лидеры"

### 4. Вкладка "Игры"
- Список игр турнира в формате разворачиваемых карточек
- При разворачивании одной игры все остальные игры автоматически сворачиваются
- Игры в статусах `Upcoming` и `Canceled` разворачивать нельзя
- Для выбранной игры показывается интерфейс ставок в зависимости от статуса:
    - **Open**: возможность проголосовать + статистика голосования
    - **Locked**: только статистика голосования
    - **Resolved**: статистика голосования + правильные ответы
- Для админа турнира на каждой игре доступны админские кнопки

### 5. Вкладка "Лидеры"
- Список ников и очков
- Текущая позиция пользователя подсвечивается

### 6. Admin Panel (отдельный URL / доступ по Telegram ID)
...
3. Показывается экран авторизации с полем ника и кнопкой "Начать".
4. Пользователь вводит игровой ник и подтверждает действие.
5. Mini App вызывает `/api/me/nickname` для сохранения ника.
6. После успешного сохранения ника открывается экран со списком турниров.
7. После выбора турнира пользователь получает доступ к вкладкам "Игры" и "Лидеры".
